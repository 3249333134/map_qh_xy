  ...
  >
    ...
    <view class="search-box" catchtouchmove="true" @touchstart="onDragStart" @touchmove.stop.prevent="onDrag" @touchend="onDragEnd" @touchcancel="onDragEnd">
      <view
        ...
        :class="{ collapsed: isCollapsed, 'detail-open': categoryActionExpanded && !isCollapsed }"
...
.search-input-wrapper { display: flex; align-items: center; width: 100%; height: 48px; padding: 0 10px 0 17px; background: #f8fafc; border: 1px solid rgba(148,163,184,.28); border-radius: 24px; box-shadow: inset 0 1px 0 rgba(255,255,255,.9); transition: width 200ms cubic-bezier(.2,.8,.2,1), background-color 200ms ease, box-shadow 200ms ease; }
.search-input-wrapper:active { background: #fff7ed; border-color: rgba(234,88,12,.24); }
...
.search-input-wrapper.detail-open { width: 96px; flex: 0 0 96px; padding-right: 12px; overflow: hidden; }
.search-input-wrapper.detail-open .ai-badge { display: none; }
...
.search-action-fixed { position: absolute; right: 0; top: 50%; transform: translateY(-50%); width: 64px; height: 48px; border-radius: 18px; background: linear-gradient(135deg,#ff7a45 0%,#f97316 48%,#ea580c 100%); border: 2px solid rgba(255,255,255,.92); box-shadow: 0 8px 24px rgba(234,88,12,.3); display: flex; align-items: center; justify-content: center; gap: 5px; color: #fff; transition: transform 160ms ease, box-shadow 160ms ease; }

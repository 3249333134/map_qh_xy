  ...
  <scroll-view class="expanded-modules" scroll-y :style="{ height: height + 'px' }" @scroll="onScroll">
    <view class="modules-header">
      <view class="header-kicker-row">
        <text class="detail-heading">{{ pointData.name || pointData.title || `${typeLabel}详情` }}</text>
    ...
    <swiper class="image-swiper" :class="{ 'is-empty': !galleryImages.length }" :indicator-dots="true" circular :autoplay="false">
      <swiper-item v-for="(img, i) in gallerySlides" :key="i">
        ...
        <view v-else class="map-placeholder">
          ...
          <view class="placeholder-copy"><text class="placeholder-label">{{ typeLabel }}影像待补充</text><text class="placeholder-hint">{{ detailType === 'service' ? '查看服务说明，选择合适时段' : detailType === 'track' ? '查看路线数据与途经节点' : '查看地点介绍与附近内容' }}</text></view>
        </view>
      </swiper-item>
    </swiper>
...
<style scoped>
.expanded-modules { width: 100%; box-sizing: border-box; background: transparent; padding-bottom: 16px; }
.modules-header { position: relative; background: transparent; padding: 16px 16px 12px; }
...
.header-kicker { font-size: 12px; color: var(--color-text-body); font-weight: 600; }
...
.header-nav { flex: 0 0 auto; min-width: 76px; min-height: 44px; padding: 0 14px; justify-content: center; gap: 6px; color: #fff; background: var(--gradient-primary); border-radius: 15px; box-shadow: 0 8px 20px rgba(0, 0, 0, 0.1); }
...
.header-address { font-size: 13px; color: var(--color-text-body); display: block; line-height: 1.5; }
.stats-row { margin-top: 8px; color: var(--color-text-body); font-size: 12px; }
.stat { font-size: 12px; }.stat.strong { color: var(--color-primary); font-weight: 700; }.dot { margin: 0 7px; color: #cbd5e1; }
.image-swiper { width: 100%; box-sizing: border-box; height: 208px; padding: 0 16px; background: transparent; }
.slide-img,.map-placeholder { width: 100%; height: 100%; overflow: hidden; border-radius: 22px; }
.slide-img { background: var(--color-border); }
.map-placeholder { position: relative; background: #eaf0ed; }
.route-line { position: absolute; height: 4px; border-radius: 4px; background: rgba(255,255,255,.9); transform-origin: left center; display: none; }
...
.place-marker { position: absolute; left: 54%; top: 38%; width: 34px; height: 34px; background: var(--color-primary); border: 4px solid #fff; border-radius: 50% 50% 50% 0; transform: rotate(-45deg); box-shadow: 0 8px 18px rgba(0, 0, 0, 0.1); display: none; }
.place-marker view { width: 10px; height: 10px; margin: 8px; border-radius: 50%; background: #fff; }
.placeholder-copy { position: absolute; display: flex; flex-direction: column; border-radius: 10px; background: transparent; padding: 12px; left: 4px; bottom: 8px; }
.placeholder-label { font-size: 13px; color: var(--color-text); font-weight: 700; }.placeholder-hint { margin-top: 2px; font-size: 11px; color: var(--color-text-body); line-height: 1.5; }
.description { padding: 16px; background: transparent; }
.desc-text { font-size: 14px; color: var(--color-text-body); line-height: 1.65; }
.typed-detail { padding: 0 16px 16px; background: transparent; }.identity-row { display: flex; align-items: center; gap: 10px; border-top: 1px solid var(--color-surface-muted); padding: 16px; background: rgba(255,255,255,.82); border-radius: 20px; margin-bottom: 20px; flex-wrap: wrap; }.identity-mark { flex: 0 0 44px; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; font-size: 16px; font-weight: 800; background: #e0f2ec; color: #286c5c; border-radius: 14px; }.identity-copy { min-width: 0; flex: 1; }.identity-title,.identity-sub,.price-text,.policy-card text,.slot-chip text,.node-copy text,.track-stats text { display: block; }.identity-title { color: var(--color-text); font-weight: 750; font-size: 15px; }.identity-sub { margin-top: 4px; color: var(--color-text-body); line-height: 1.6; font-size: 11px; }.price-text { font-weight: 800; font-size: 24px; color: #263d32; }.price-text text { display: inline; font-size: 11px; font-weight: 600; }
.typed-title { display: block; margin: 14px 0 10px; color: var(--color-text); font-size: 16px; font-weight: 600; margin-bottom: 12px; }.slot-list { overflow-x: auto; display: grid; grid-template-columns: repeat(3,minmax(0,1fr)); gap: 8px; }.slot-chip { flex: 0 0 64px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: var(--color-text-body); padding: 10px 6px; min-height: 60px; background: rgba(255,255,255,.8); border: 1px solid transparent; border-radius: 16px; }.slot-chip text { font-size: 11px; line-height: 18px; }.slot-chip.active { background: #e0f2ec; color: #286c5c; border-color: #6b9b86; }.slot-chip.disabled { opacity: .38; }
.policy-card { margin-top: 14px; padding: 14px 0; background: transparent; border-radius: 0; }.policy-card text { font-size: 12px; line-height: 1.8; color: var(--color-text-body); }.primary-action { display: flex; align-items: center; justify-content: center; font-weight: 750; min-height: 48px; border-radius: 16px; background: #263d32; color: #fff; font-size: 14px; box-shadow: none; margin-top: 0; }.primary-action.disabled { opacity: .42; box-shadow: none; }
.track-stats { display: grid; grid-template-columns: repeat(3,1fr); padding: 14px 0; border-top: 1px solid var(--color-surface-muted); gap: 8px; margin-bottom: 20px; }.track-stats view { min-height: 66px; display: flex; flex-direction: column; align-items: center; justify-content: center; background: rgba(255,255,255,.84); padding: 16px 8px; border-radius: 18px; }.track-stats text:first-child { color: var(--color-text); font-size: 24px; font-weight: 650; }.track-stats text:last-child { margin-top: 3px; color: var(--color-text-body); font-size: 10px; }.node-row { display: flex; align-items: center; gap: 10px; min-height: 64px; background: transparent; border-bottom: 1px solid var(--color-divider); }.node-index { flex: 0 0 30px; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 750; background: #e0f2ec; color: #286c5c; border-radius: 50%; }.node-copy { flex: 1; }.node-copy text:first-child { color: var(--color-text); font-size: 13px; font-weight: 700; }.node-copy text:last-child { margin-top: 3px; color: var(--color-text-body); font-size: 11px; }.chevron { width: 7px; height: 7px; margin-right: 6px; border-top: 2px solid var(--color-text-muted); border-right: 2px solid var(--color-text-muted); transform: rotate(45deg); }
.subtabs { display: flex; padding: 4px 16px 14px; background: #fff; }
.subtab { min-height: 36px; padding: 0 16px; margin-right: 8px; border-radius: 18px; background: var(--color-surface-muted); color: var(--color-text-body); font-size: 13px; display: flex; align-items: center; justify-content: center; }
.subtab.active { font-weight: 650; box-shadow: none; background: #e0f2ec; color: #286c5c; }
.module { padding: 16px 16px 0; }.module:last-child { padding-bottom: 24px; }
.module-title { padding-bottom: 10px; color: var(--color-text); font-size: 16px; font-weight: 700; }
.module-list { display: flex; flex-direction: column; gap: 10px; }
.list-item { display: flex; align-items: center; gap: 10px; min-height: 76px; background: #fff; flex-wrap: wrap; border: 0; border-radius: 18px; padding: 12px; box-shadow: none; }
.item-cover { position: relative; flex: 0 0 52px; width: 52px; height: 52px; border-radius: 12px; overflow: hidden; display: flex; align-items: center; justify-content: center; background: linear-gradient(145deg,var(--color-primary-soft),#ffd7df); color: var(--color-primary); }
...
.item-left { min-width: 0; flex: 1; display: flex; flex-direction: column; }.item-name { font-size: 14px; color: var(--color-text); font-weight: 650; }.item-desc { font-size: 12px; color: var(--color-text-body); margin-top: 4px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }
.item-actions { flex: 0 0 auto; display: flex; gap: 6px; align-items: center; width: 100%; justify-content: flex-end; flex-wrap: wrap; }.price { color: var(--color-primary); font-size: 14px; font-weight: 750; }
.btn-secondary,.btn-primary { min-width: 48px; margin: 0; padding: 0 10px; min-height: 44px; line-height: 44px; border-radius: 14px; font-size: 12px; }
.btn-secondary { background: #fff; color: var(--color-primary-strong); border: 1px solid rgba(32,32,32,.18); }.btn-primary { background: var(--gradient-primary); color: #fff; border: none; }
.btn-secondary::after,.btn-primary::after { border: none; }.btn-secondary:active,.btn-primary:active { opacity: .78; }.detail-heading { font-size: 20px; line-height: 1.3; font-weight: 650; color: var(--color-text); flex: 1; min-width: 0; }.image-swiper.is-empty { height: 112px; }.detail-action-dock { background: rgba(250,253,252,.92); backdrop-filter: blur(24px) saturate(125%); -webkit-backdrop-filter: blur(24px) saturate(125%); box-shadow: var(--shadow-float); position: sticky; bottom: 0; padding: 10px; border-radius: 22px; z-index: 3; }.secondary-actions { display: flex; gap: 8px; margin-bottom: 8px; }.secondary-actions > view { flex: 1; min-height: 40px; display: flex; align-items: center; justify-content: center; font-size: 12px; border-radius: 12px; background: #edf3f0; color: #385448; }.place-tools { display: flex; gap: 8px; padding: 0 16px 16px; }.place-tools > view { min-height: 40px; flex: 1; display: flex; align-items: center; justify-content: center; background: rgba(255,255,255,.8); border-radius: 14px; font-size: 12px; }
</style>

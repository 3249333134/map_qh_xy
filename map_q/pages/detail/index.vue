<template>
  <view class="detail-page-wrapper">
    ...
    <template v-else-if="detail">
      ...
      <VideoDetail v-else-if="cardType === 'video'" :detail="detail" @active-change="detail = $event" />
      <NormalDetail v-else-if="cardType === 'article'" />
      ...
      <NormalDetail v-else />
      <DetailCommonOverlay v-if="cardType !== 'video'" :detail="detail" @state-change="actionState = $event" />
    </template>
  </view>
</template>
...
async function loadDetail(force = false) {
  ...
  try {
    ...
    if (['place', 'track'].includes(value.type) && options.inline !== '0') openMapContext(value)
  } catch (cause) {
...
.error-state { min-height: 100vh; padding: 140px 28px 40px; display: flex; flex-direction: column; align-items: center; box-sizing: border-box; text-align: center; }.error-mark { width: 58px; height: 58px; display: flex; align-items: center; justify-content: center; border-radius: 18px; color: #b91c1c; background: #fef2f2; font-size: 28px; font-weight: 800; }.error-title { margin-top: 18px; color: #182033; font-size: 20px; font-weight: 750; }.error-copy { margin-top: 8px; color: #5c667a; font-size: 14px; line-height: 1.6; }.retry-button { min-width: 132px; min-height: 46px; margin-top: 24px; display: flex; align-items: center; justify-content: center; border-radius: 15px; color: #fff; background: var(--color-primary); font-size: 14px; font-weight: 700; }

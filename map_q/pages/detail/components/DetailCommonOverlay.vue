...
<style scoped>
.more-trigger { position: fixed; z-index: 1200; right: 12px; top: calc(env(safe-area-inset-top) + 10px); width: 44px; height: 44px; border-radius: 22px; display: flex; align-items: center; justify-content: center; gap: 3px; background: rgba(255,255,255,.92); box-shadow: 0 8px 24px rgba(0, 0, 0, 0.1); }.dot { width: 4px; height: 4px; border-radius: 50%; background: var(--color-text); }
.scrim { position: fixed; z-index: 1290; inset: 0; background: rgba(15,23,42,.48); }.action-sheet { position: fixed; z-index: 1300; left: 0; right: 0; bottom: 0; padding: 8px 16px calc(18px + env(safe-area-inset-bottom)); border-radius: 28px 28px 0 0; background: #fff; box-shadow: 0 -16px 40px rgba(0, 0, 0, 0.18); animation: sheetIn .22s ease-out; }
...
.sheet-title,.sheet-subtitle,.row-copy text { display: block; }.sheet-title { color: var(--color-text); font-size: 20px; font-weight: 750; }.sheet-subtitle { margin-top: 3px; color: var(--color-text-body); font-size: 12px; }
.close-button { position: relative; width: 44px; height: 44px; border-radius: 15px; background: var(--color-surface-muted); }.close-button::before,.close-button::after { content: ''; position: absolute; left: 14px; top: 21px; width: 16px; height: 2px; border-radius: 2px; background: var(--color-text-body); transform: rotate(45deg); }.close-button::after { transform: rotate(-45deg); }
.copyright-card { gap: 12px; margin-bottom: 10px; padding: 13px; border: 1px solid #ffd7df; border-radius: 16px; background: #fffaf5; }.copyright-mark,.row-icon { flex: 0 0 44px; width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; border-radius: 14px; }.copyright-mark { color: var(--color-primary); background: var(--color-primary-soft); font-size: 18px; font-weight: 800; }
.row-copy { flex: 1; min-width: 0; }.row-copy text:first-child { color: var(--color-text); font-size: 14px; font-weight: 700; }.row-copy text:last-child { margin-top: 3px; color: var(--color-text-body); font-size: 12px; line-height: 1.45; }.action-row { gap: 12px; min-height: 66px; border-bottom: 1px solid var(--color-surface-muted); }.action-row:last-child { border-bottom: 0; }.row-icon { position: relative; background: var(--color-primary-soft); }.action-row.danger .row-icon { color: #b91c1c; background: #fef2f2; font-weight: 800; }
.channel-icon::before { content: ''; width: 18px; height: 14px; border: 2px solid var(--color-primary); border-radius: 5px; box-shadow: 4px 4px 0 -2px #eaf8f5,4px 4px 0 0 #16b8a7; }.location-icon::before { content: ''; width: 15px; height: 15px; border: 2px solid var(--color-primary); border-radius: 50% 50% 50% 0; transform: rotate(-45deg); }.hide-icon::before { content: ''; width: 19px; height: 12px; border: 2px solid var(--color-text-body); border-radius: 50%; }.hide-icon::after { content: ''; position: absolute; width: 23px; height: 2px; background: var(--color-text-body); transform: rotate(-35deg); }
.chevron { width: 8px; height: 8px; margin-right: 6px; border-top: 2px solid var(--color-text-muted); border-right: 2px solid var(--color-text-muted); transform: rotate(45deg); }@keyframes sheetIn { from { opacity: 0; transform: translateY(22px); } to { opacity: 1; transform: translateY(0); } }@media (prefers-reduced-motion: reduce) { .action-sheet { animation: none; } }
</style>

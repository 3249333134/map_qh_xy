...
<style scoped>
.redirect-page { min-height: 100vh; display: flex; align-items: center; justify-content: center; background: var(--color-page); }.loader,.error-state { display: flex; flex-direction: column; align-items: center; color: #5c667a; font-size: 13px; }.loader view { width: 34px; height: 34px; margin-bottom: 14px; border: 3px solid #ffd7df; border-top-color: var(--color-primary); border-radius: 50%; animation: spin .8s linear infinite; }.error-state view { min-width: 120px; min-height: 46px; margin-top: 18px; border-radius: 15px; display: flex; align-items: center; justify-content: center; color: #fff; background: #f56f64; font-weight: 700; }@keyframes spin { to { transform: rotate(360deg); } }@media (prefers-reduced-motion: reduce) { .loader view { animation: none; } }
</style>

...
const draft = reactive({
  ...
  category: 'food', selectionSnapshot: {}, contact: { name: '', phone: '' }, location: { address: '' }, policyAccepted: false
})
const steps = [{ id: 1, name: '时间' }, { id: 2, name: '规格' }, { id: 3, name: '联系' }, { id: 4, name: '确认' }]
const specifications = computed(() => service.value.service?.packages?.map(item => item.name) || ['标准服务'])
...
function syncSelectionSnapshot() {
  const selected = service.value.service?.packages?.find(item => item.name === draft.specification) || service.value.service?.packages?.[0]
  draft.selectionSnapshot = { ...(draft.selectionSnapshot || {}), packageId: selected?.id || '', quantity: draft.quantity }
}
...
onLoad(async options => {
  ...
  const normalized = normalizeService({ ...raw, ...service.value, ...(service.value.service || {}) }, options?.category || raw.serviceCategory)
  service.value = { ...service.value, ...normalized, service: { ...(service.value.service || {}), category: normalized.serviceCategory, packages: normalized.packages, bookingConfig: normalized.bookingConfig, venue: normalized.venue, pricing: normalized.pricing, availableSlots: normalized.availableSlots, policies: normalized.policies } }
  draft.serviceId = service.value.id
  draft.category = normalized.serviceCategory
  draft.quantity = Number(raw.people || 1)
  draft.specification = raw.selectedPackage?.name || normalized.packages[0]?.name || ''
  draft.selectionSnapshot = { packageId: raw.selectedPackage?.id || normalized.packages[0]?.id || '', quantity: draft.quantity, duration: raw.selectedDuration || 1 }
...
<style scoped>
.booking-page { min-height: 100vh; background: var(--color-page); color: var(--color-text); }.nav-back { width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; border-radius: 15px; background: var(--color-surface-muted); }.back-icon { width: 10px; height: 10px; border-left: 2px solid var(--color-text); border-bottom: 2px solid var(--color-text); transform: rotate(45deg); }.page-content { padding: 14px 14px 118px; box-sizing: border-box; }
.summary-card,.progress-card,.form-card { margin-bottom: 12px; border: 1px solid var(--color-border); border-radius: 20px; background: #fff; box-shadow: 0 8px 26px rgba(0, 0, 0, 0.05); }.summary-card { display: flex; align-items: center; gap: 12px; padding: 14px; }.service-mark { flex: 0 0 48px; width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 15px; color: #fff; background: var(--color-primary); font-size: 16px; font-weight: 800; }.summary-copy { min-width: 0; }.booking-title,.booking-desc { display: block; }.booking-title { font-size: 17px; font-weight: 750; }.booking-desc { margin-top: 4px; color: var(--color-text-body); font-size: 12px; }
.progress-card { display: grid; grid-template-columns: repeat(4,1fr); padding: 14px 8px; }.progress-item { position: relative; display: flex; flex-direction: column; align-items: center; color: var(--color-text-muted); font-size: 11px; }.progress-item::before { content: ''; position: absolute; top: 13px; right: 50%; width: 100%; height: 2px; background: var(--color-border); }.progress-item:first-child::before { display: none; }.progress-item.active::before { background: var(--color-primary); }.progress-dot { position: relative; z-index: 1; width: 26px; height: 26px; margin-bottom: 5px; display: flex; align-items: center; justify-content: center; border-radius: 50%; background: var(--color-border); color: var(--color-text-body); font-size: 11px; font-weight: 750; }.progress-item.active .progress-dot { color: #fff; background: var(--color-primary); }.progress-item.current { color: var(--color-primary); font-weight: 700; }
.form-card { padding: 18px 16px; }.section-title,.section-hint,.field-label,.field-help,.field-error,.input-count { display: block; }.section-title { font-size: 19px; font-weight: 800; }.section-hint { margin-top: 6px; color: var(--color-text-body); font-size: 12px; line-height: 1.5; }.field-block { padding: 18px 0; border-bottom: 1px solid var(--color-surface-muted); }.field-block.no-border { border-bottom: 0; }.field-label { color: var(--color-text); font-size: 14px; font-weight: 650; }.required,.field-error { color: #b91c1c; }.field-error { margin-top: 7px; font-size: 12px; }
.date-scroll { margin-top: 12px; white-space: nowrap; }.date-list { display: inline-flex; gap: 8px; padding-right: 12px; }.date-chip { width: 58px; height: 58px; border: 1px solid var(--color-border); border-radius: 14px; display: inline-flex; flex-direction: column; align-items: center; justify-content: center; color: var(--color-text-body); }.date-chip text { font-size: 11px; line-height: 18px; }.date-chip.active { border-color: var(--color-primary); color: var(--color-primary); background: var(--color-primary-soft); }.time-grid { margin-top: 12px; display: grid; grid-template-columns: repeat(3,1fr); gap: 9px; }.time-chip,.spec-chip { min-height: 44px; display: flex; align-items: center; justify-content: center; border: 1px solid var(--color-border); border-radius: 13px; color: var(--color-text-body); font-size: 13px; }.time-chip.active,.spec-chip.active { border-color: var(--color-primary); color: var(--color-primary); background: var(--color-primary-soft); }.time-chip.disabled { opacity: .38; }
.field-row { min-height: 68px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--color-surface-muted); }.field-help { margin-top: 4px; color: var(--color-text-muted); font-size: 11px; }.counter { display: flex; align-items: center; gap: 15px; }.counter view { width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; border-radius: 13px; background: var(--color-surface-muted); color: var(--color-text); font-size: 20px; }.counter text { min-width: 22px; text-align: center; font-weight: 750; }.spec-list { margin-top: 12px; display: grid; grid-template-columns: repeat(3,1fr); gap: 8px; }.note-input { width: 100%; min-height: 100px; margin-top: 10px; padding: 12px; border-radius: 14px; background: var(--color-page); box-sizing: border-box; font-size: 14px; }.input-count { margin-top: 5px; color: var(--color-text-muted); font-size: 11px; text-align: right; }
.input-group { display: block; padding: 14px 0 10px; border-bottom: 1px solid var(--color-surface-muted); }.input-group>text { display: block; color: var(--color-text); font-size: 13px; font-weight: 650; }.input-group input,.input-group textarea { width: 100%; min-height: 44px; margin-top: 6px; font-size: 15px; }.textarea-group textarea { min-height: 76px; }.privacy-tip { margin-top: 16px; padding: 12px; display: flex; align-items: center; gap: 9px; border-radius: 13px; color: var(--color-text-body); background: #f0fdfa; font-size: 11px; }.shield-icon { width: 16px; height: 18px; border: 2px solid var(--color-primary); border-radius: 8px 8px 10px 10px; }
.receipt,.policy-list { margin-top: 14px; padding: 12px; border-radius: 14px; background: var(--color-page); }.receipt view,.policy-list view { min-height: 32px; display: flex; justify-content: space-between; gap: 18px; color: var(--color-text-body); font-size: 12px; }.receipt .total { padding-top: 8px; border-top: 1px solid var(--color-border); color: var(--color-text); font-size: 15px; font-weight: 750; }.receipt .total text:last-child { color: var(--color-primary); }.policy-list view { display: block; padding: 8px 0; border-bottom: 1px solid var(--color-border); }.policy-list view:last-child { border-bottom: 0; }.policy-list text { display: block; }.policy-list text:first-child { color: var(--color-text-body); font-weight: 700; }.policy-list text:last-child { margin-top: 4px; line-height: 1.5; }.agreement { min-height: 54px; display: flex; align-items: center; gap: 10px; color: var(--color-text); font-size: 12px; }.check-box { width: 22px; height: 22px; border: 2px solid #cbd5e1; border-radius: 7px; box-sizing: border-box; }.check-box.checked { position: relative; border-color: var(--color-primary); background: var(--color-primary); }.check-box.checked::after { content: ''; position: absolute; left: 5px; top: 2px; width: 6px; height: 11px; border-right: 2px solid #fff; border-bottom: 2px solid #fff; transform: rotate(45deg); }.demo-notice { padding: 11px; border-radius: 12px; color: #9a3412; background: var(--color-primary-soft); font-size: 11px; }
.booking-footer { position: fixed; z-index: 1000; left: 0; right: 0; bottom: 0; min-height: 78px; padding: 10px 14px; display: flex; align-items: center; gap: 9px; background: rgba(255,255,255,.97); border-top: 1px solid var(--color-border); box-sizing: content-box; backdrop-filter: blur(10px); }.price-area { flex: 1; display: flex; flex-direction: column; }.price { color: var(--color-primary); font-size: 20px; font-weight: 800; }.price-note { color: var(--color-text-body); font-size: 10px; }.previous-btn,.confirm-btn { min-width: 72px; height: 48px; padding: 0 13px; display: flex; align-items: center; justify-content: center; border-radius: 15px; font-size: 13px; font-weight: 700; }.previous-btn { color: var(--color-text-body); background: var(--color-surface-muted); }.confirm-btn { min-width: 112px; color: #fff; background: var(--color-primary); box-shadow: 0 7px 18px rgba(0, 0, 0, 0.1); }.confirm-btn.disabled { opacity: .45; box-shadow: none; }
.success-state { padding: 120px 24px 40px; text-align: center; }.success-mark { width: 72px; height: 72px; margin: 0 auto 18px; border-radius: 23px; display: flex; align-items: center; justify-content: center; background: #dcfce7; }.success-mark view { width: 18px; height: 34px; border-right: 5px solid #15803d; border-bottom: 5px solid #15803d; transform: rotate(45deg) translate(-4px,-4px); }.success-title,.success-desc,.success-copy { display: block; }.success-title { font-size: 22px; font-weight: 800; }.success-desc { margin-top: 10px; font-size: 15px; font-weight: 650; }.success-copy { margin-top: 10px; color: #5c667a; font-size: 13px; line-height: 1.65; }.success-actions { margin-top: 30px; display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }.secondary-btn,.primary-btn { height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 15px; font-size: 14px; font-weight: 750; }.secondary-btn { color: var(--color-text-body); background: var(--color-surface-muted); }.primary-btn { color: #fff; background: var(--color-primary); }
</style>

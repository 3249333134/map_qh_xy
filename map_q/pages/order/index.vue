  ...
  <view class="page">
    ...
    <view class="content" :style="{ paddingTop: topOffset + 'px' }">
      ...
      <template v-else>
        ...
        <view v-if="showVerification" class="card verification-card">
          <view class="verification-head"><view><text class="section-title">到店核销码</text><text>请到店后向商家出示，仅限本订单使用</text></view><text class="verification-status">{{ order.verificationStatus === 'verified' ? '已核销' : '待使用' }}</text></view>
          <view class="code-grid" :class="{used:order.verificationStatus==='verified'}"><view v-for="cell in codeCells" :key="cell.key" :class="{dark:cell.dark}"></view></view>
          <text class="verification-code">{{ spacedCode }}</text>
          <text v-if="order.verifiedAt" class="verified-time">核销时间：{{ formatTime(order.verifiedAt) }}</text>
          <view v-if="order.verificationStatus!=='verified'" class="verify-action" @tap="verifyAtStore">模拟商家确认核销</view>
        </view>
        <view class="card">
...
const showVerification = computed(() => [STATUS.CONFIRMED, STATUS.PENDING_FULFILLMENT, STATUS.IN_SERVICE, STATUS.COMPLETED].includes(order.value?.status))
const spacedCode = computed(() => String(order.value?.verificationCode || '').replace(/(\d{3})(\d{3})/, '$1 $2'))
const codeCells = computed(() => Array.from({ length: 81 }, (_, index) => ({ key: index, dark: ((index * 7 + Number(String(order.value?.verificationCode || '0')[index % 6] || 0)) % 5) < 2 || index < 9 || index % 9 === 0 })))
...
function verifyAtStore() {
  uni.showModal({ title: '模拟商家核销', editable: true, placeholderText: '请输入订单上的 6 位核销码', content: order.value.verificationCode, confirmText: '确认核销', success: result => {
    if (!result.confirm) return
    try { order.value = bookingApi.verify(order.value.id, result.content || order.value.verificationCode) } catch (cause) { uni.showToast({ title: cause.message, icon: 'none' }) }
  } })
}
...
<style scoped>
.page { min-height: 100vh; background: var(--color-page); }.back-button { width: 44px; height: 44px; border-radius: 15px; display: flex; align-items: center; justify-content: center; background: var(--color-surface-muted); }.back-button view { width: 10px; height: 10px; border-left: 2px solid var(--color-text); border-bottom: 2px solid var(--color-text); transform: rotate(45deg); }.content { padding: 14px 14px 34px; }.status-card,.card,.action-card { margin-bottom: 12px; padding: 16px; border: 1px solid #e3e9f3; border-radius: 20px; background: #fff; box-shadow: 0 8px 24px rgba(15,23,42,.05); }
.status-card { display: flex; align-items: center; gap: 13px; }.status-icon { width: 48px; height: 48px; border-radius: 16px; display: flex; align-items: center; justify-content: center; background: #fff1ee; }.status-icon view { width: 18px; height: 18px; border: 3px solid #f56f64; border-radius: 50%; box-sizing: border-box; box-shadow: 0 0 0 5px rgba(234,88,12,.12); }.status-title,.status-desc,.card-title,.section-title,.timeline-row text,.review-text,.review-lock { display: block; }.status-title { color: #182033; font-size: 18px; font-weight: 800; }.status-desc { margin-top: 4px; color: #5c667a; font-size: 12px; }.card-title,.section-title { margin-bottom: 10px; color: var(--color-text); font-size: 16px; font-weight: 750; }
.info-row { min-height: 42px; display: flex; justify-content: space-between; gap: 18px; border-bottom: 1px solid var(--color-surface-muted); color: var(--color-text-body); font-size: 12px; }.info-row text { display: flex; align-items: center; }.info-row text:last-child { max-width: 66%; color: var(--color-text-body); text-align: right; }.info-row.total { border-bottom: 0; color: var(--color-text); font-weight: 700; }.info-row.total text:last-child { color: var(--color-primary); font-size: 17px; font-weight: 800; }
.timeline-row { position: relative; min-height: 52px; padding-left: 30px; }.timeline-row::before { content: ''; position: absolute; left: 8px; top: 15px; bottom: -15px; width: 2px; background: var(--color-border); }.timeline-row:last-child::before { display: none; }.timeline-mark { position: absolute; left: 2px; top: 8px; width: 14px; height: 14px; border: 3px solid #fff; border-radius: 50%; background: var(--color-primary); box-shadow: 0 0 0 2px #ffd7df; }.timeline-row text:first-child { color: var(--color-text); font-size: 13px; font-weight: 650; }.timeline-row text:last-child { margin-top: 4px; color: var(--color-text-muted); font-size: 10px; }
.rating-row { display: flex; gap: 10px; }.rating-row>view { width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; border-radius: 14px; background: #f1f5f9; }.rating-row>view view { width: 18px; height: 18px; clip-path: polygon(50% 0,61% 35%,98% 35%,68% 57%,79% 94%,50% 72%,21% 94%,32% 57%,2% 35%,39% 35%); background: #cbd5e1; }.rating-row>view.active { background: #fff1ee; }.rating-row>view.active view { background: #f59e0b; }.review-input { width: 100%; min-height: 100px; margin-top: 12px; padding: 12px; border-radius: 14px; background: var(--color-page); box-sizing: border-box; font-size: 14px; }.review-text { color: var(--color-text); font-size: 14px; line-height: 1.65; }.review-lock { margin-top: 10px; color: var(--color-text-body); font-size: 11px; }
.action-card { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px; }.secondary-action,.primary-action { min-height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 15px; font-size: 13px; font-weight: 750; }.secondary-action { color: var(--color-text-body); background: var(--color-surface-muted); }.secondary-action.danger { color: #b91c1c; background: #fef2f2; }.primary-action { color: #fff; background: var(--color-primary); box-shadow: 0 7px 18px rgba(0, 0, 0, 0.1); }.empty-state { padding-top: 140px; text-align: center; }.empty-state text { display: block; color: var(--color-text-body); }.empty-state view { width: 120px; height: 46px; margin: 20px auto; display: flex; align-items: center; justify-content: center; border-radius: 15px; color: #fff; background: var(--color-primary); }
.verification-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }.verification-head .section-title { margin-bottom: 4px; }.verification-head>view>text:last-child { color: #65706d; font-size: 10px; }.verification-status { padding: 6px 9px; border-radius: 9px; color: var(--color-primary); background: var(--color-primary-soft); font-size: 10px; font-weight: 750; }.code-grid { width: 144px; height: 144px; margin: 18px auto 12px; padding: 8px; border-radius: 12px; display: grid; grid-template-columns: repeat(9,1fr); grid-template-rows: repeat(9,1fr); gap: 2px; background: #fff; box-shadow: none; }.code-grid view { border-radius: 1px; }.code-grid .dark { background: var(--color-text); }.code-grid.used { opacity: .3; }.verification-code { display: block; text-align: center; color: var(--color-text); font-size: 24px; font-weight: 850; letter-spacing: 5px; font-variant-numeric: tabular-nums; }.verified-time { display: block; margin-top: 8px; color: #65706d; text-align: center; font-size: 10px; }.verify-action { height: 48px; margin-top: 16px; border-radius: 15px; display: flex; align-items: center; justify-content: center; color: #fff; background: var(--gradient-primary); font-size: 13px; font-weight: 750; }
</style>
